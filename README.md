# 诗云 · Verse Cloud

一个可运行的诗词空间应用：在 WebGL 唐代疆域与真实地形晕渲上游历中国古典诗词，并让长安、江南和西域三层程序化音景随地图中心自然交融。

## 本地运行

```bash
npm install
npm run dev
```

质量检查：

```bash
npm run typecheck
npm test
npm run test:e2e
npm run build
```

端到端测试默认使用本机 Chrome，并由 Playwright 自动启动 Vite 开发服务器。

## 当前版本

- React、TypeScript、MapLibre GL WebGL 地图和 D3 音景空间投影。
- Natural Earth 真实地形晕渲，以及 OpenFreeMap / OpenStreetMap 水系。
- 连续显示唐代及周边的真实山河，不绘制朝代疆域填色、外边界或疆域外遮罩，并放置 8 个诗词空间节点。
- 朱雀仿宋自托管网页字体，以及直接悬于诗词地点上方、从右向左的古籍式竖排全文；诗文按标点拆句，每句独占一条竖列。
- 可俯仰、旋转、缩放的地图漫游，以及从全域山河自然落向诗词节点的入场镜头。
- 带落影、厚边、亮面和高光的立体诗词地点印，以及覆盖圆点和地名的大尺寸 WebGL 点击热区；同址多诗会自动合并为带数量的叠层诗簇，点击后以诗题选择环展开。
- 为八首诗分别设计花影烽烟、彩云轻舟、月落渔火、烟渚江月、白日黄河、黄鹤白云、紫烟瀑布和朝雨柳色地图动画。
- 选诗后镜头会平滑聚焦，把地点置于视野下部，并在其正上方统一显示完整竖排诗词。
- 滚轮拉近时，当前诗签保持可见并沿固定轨迹渐进吸附到地图中央，同时随缩放适度放大；反向拉远会沿同一轨迹自然返回并缩小。桌面与移动端分别限制最大比例，避免越出视口。
- 诗签会根据最长诗句、总句数和当前屏幕宽高动态计算字号，窗口变化时重新排版，并强制保持每句不折行；标题、作者与地点落款共享同一列高并分别缩放，避免越界。
- 左侧时空坐标跟随当前诗切换公元年份和年号；无法精确断年的作品明确显示为约年，时间轴光点同步移动。
- 唐代概念道界、远景道名与近景州名，并对海域、湖泊和江河分别设色。
- 春、夏、秋、冬四套手动场景配色；切换只更新已有地图图层，不运行全屏天气粒子。
- 地图始终只渲染当前诗的一份意象动画，共 8 个轻量 CSS 粒子；移动端进一步减少历史地名数量。
- 地图视野、瓦片请求和交互范围均限制在唐代及其周边；真实地形连续铺满视口，但无法拖动或缩放到全球。
- 道名和州名预先绘制为仿宋纹理并由 WebGL symbol 图层合批渲染；拖动期间暂停地图意象动画。
- 默认采用高性能地形晕渲，不创建实时高程网格；画布像素比最高 1.5，并限制瓦片缓存。
- 桌面和移动端均采用单一地图阅读界面，不再加载右侧诗卷、底部图例、操作提示或版本说明。
- 基于 Web Audio / Tone.js 的长安、江南、西域三路实时环境层，以及为八首诗分别编排、以双音轨自然交叉淡化的程序化诗曲。
- 八首诗分别绑定独立的调式音高、速度、节奏网格、固定音色、滤波包络和空间回声，不受选诗顺序影响；区域环境层保持在主旋律之后。
- 音频引擎限制并复用少量声部；非活动诗曲停止调度，静音会暂停 Transport 和 AudioContext，避免长时间浏览后音频节点持续增长。
- 诗词地点关系、证据说明和置信度字段。
- 桌面及移动端布局、减少动态效果偏好和浏览器自动播放授权流程。

## 创作定位

`src/data/mapSnapshots.ts` 仍保留唐代概念边界数据，供未来编辑和版本对照使用，但当前地图不渲染任何朝代外边界、疆域填色或疆域外遮罩。`src/data/tangGeography.ts` 中的道界和地名仅作为艺术化阅读层，不主张复原任一具体年份的学术政区。

诗词文本目前以 `chinese-poetry` 项目为种子，并附有概念版人工地点说明。地点分为创作地、作品场景、行旅节点、诗中提及和人物关联地，不能互相混用。

## 目录

```text
src/components/VerseScene.tsx  唐域 WebGL 图层、诗词节点与地图交互
src/data/poems.ts              诗词及地理证据样本
src/data/mapSnapshots.ts       年代快照与概念边界
src/data/tangGeography.ts      唐代概念道界、道名与州名阅读层
src/lib/geo.ts                 D3 音景空间投影
src/lib/soundscape.ts          音景权重与 Web Audio 调度
tests/e2e/                     浏览器验收测试
```

## 下一阶段

1. 按同一艺术语言绘制宋、元、明概念疆域，建立多朝代空间快照。
2. 建立 PostgreSQL/PostGIS 数据库及诗人、作品、历史地点实体关联管线。
3. 将程序化音景替换或叠加为经过审核的 AI 音乐 stems。
4. 继续压缩和按需加载自托管仿宋字体，降低首次访问体积。
5. 增加编辑审核后台，记录每一处地理判断的证据和版本。

## 地图与字体来源

- 地表晕渲：Natural Earth，经 OpenFreeMap 提供。
- 水系矢量瓦片：OpenFreeMap / OpenStreetMap contributors。
- 中文字体：朱雀仿宋（Zhuque Fangsong），按 SIL Open Font License 1.1 使用并随应用自托管。

地图中的唐代边界是艺术化概念图层；真实地形晕渲不代表边界具有学术复原精度。
